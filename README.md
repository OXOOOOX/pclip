...
One-line install for Debian/Ubuntu systems:

```bash
curl -fsSL https://raw.githubusercontent.com/OXOOOOX/pclip/main/install.sh | bash
```
...
curl -fsSL https://raw.githubusercontent.com/OXOOOOX/pclip/main/pclip | sudo tee /usr/local/bin/pclip >/dev/null
sudo chmod 0755 /usr/local/bin/pclip
```
...
2. `xclip -selection clipboard` for X11
3. `xsel --clipboard --input` for X11 fallback
